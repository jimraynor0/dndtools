...
public class MapGridLoader {
    ...
    public static MapGridCell[][] createMapGrid(String content) {
        ...
        List<List<MapGridCell>> mapGrid = new ArrayList<List<MapGridCell>>(lines.length);
        ...
        for (String line : lines) {
            ...
            for (int i = 0; i < parsed.length; i++) {
                ...
                if (color != null) {
                    ...
                    while (!segment.isEmpty() && !("" + STYLE_END).equals(segment)) {
                        String cellValue = null;
                        char ch = segment.charAt(0);
                        if ((int) ch < 256) {
                            cellValue = "" + ch + (segment.length() > 1 ? segment.charAt(1) : " ");
                        } else {
                            cellValue = "" + ch;
                        }

                        MapGridCell cell = new MapGridCell(ColorCode.toColor(color.getForeground()), ColorCode.toColor(color
                                .getBackground()), cellValue);
        ...
        MapGridCell[][] grid = new MapGridCell[mapGrid.get(0).size()][mapGrid.size()];
        ...
        try {
            ...
            try {
                ...
                if (foregroundCode >= ColorCode.getColorCount()) {
                    ...
                    color.setForeground(Integer.parseInt(foreground.substring(0, foregroundCodeLength)));
                } else {
            ...
            if (str.length() >= foregroundCodeLength + 3 && ',' == str.charAt(foregroundCodeLength)) {
                String background = str.substring(foregroundCodeLength + 1, foregroundCodeLength + 3);
                try {
